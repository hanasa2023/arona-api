...
import { getRandomSamples, parseRedisHash } from '@/utils/tools'
...
export class Database {
  ...
  async getIllusts(req: IRequest): Promise<Illust[]> {
    ...
    for (const pid of pids) {
      ...
      if (!illustInfo.loveMembers)
        await this._client.hSet(`baId:${pid}`, 'loveMember', 0)
      if (!illustInfo.hateMembers)
        await this._client.hSet(`baId:${pid}`, 'hateMember', 0)
      illusts.push({
        pid: illustInfo.id,
        ...
        loveMembers: illustInfo.loveMembers,
        hateMembers: illustInfo.hateMembers,
      })
    }
  ...
  async setIllusts(pid: string): Promise<string> {
    ...
    if (response.ok) {
      ...
      if (!existsId) {
        ...
        await this._client.hSet(`baId:${pid}`, 'loveCount', 1)
        await this._client.hSet(`baId:${pid}`, 'hateCount', 0)
  ...
  async updateLoveMembers(pid: string, type: string): Promise<string> {
    await this.connect()
    const loveMembers =
      Number(await this._client.hGet(`baId:${pid}`, 'loveMembers')) ?? 0
    try {
      let updateNumber = 0
      if (type === 'remove') {
        updateNumber = loveMembers - 1 <= 0 ? 0 : loveMembers - 1
      } else {
        updateNumber = loveMembers + 1
      }
      const existsId = await this._client.exists(`baId:${pid}`)
      if (existsId) {
        await this._client.hSet(`baId:${pid}`, 'loveMembers', updateNumber)
        await this.disconnect()
        return type === 'remove' ? '取消点赞成功' : '点赞成功'
      } else {
        await this.disconnect()
        return '图片不存在'
      }
    } catch (e) {
      return type === 'remove' ? '取消点赞失败' : '点赞失败'
    }
  }

  async updateHateMembers(pid: string, type: string): Promise<string> {
    await this.connect()
    const hateMembers =
      Number(await this._client.hGet(`baId:${pid}`, 'hateMembers')) ?? 0
    try {
      let updateNumber = 0
      if (type === 'remove') {
        updateNumber = hateMembers - 1 <= 0 ? 0 : hateMembers - 1
      } else {
        updateNumber = hateMembers + 1
      }
      const existsId = await this._client.exists(`baId:${pid}`)
      if (existsId) {
        await this._client.hSet(`baId:${pid}`, 'hateMembers', updateNumber)
        await this.disconnect()
        return type === 'remove' ? '取消踩成功' : '踩成功'
      } else {
        await this.disconnect()
        return '图片不存在'
      }
    } catch (e) {
      return type === 'remove' ? '取消踩失败' : '踩失败'
    }
  }
}
